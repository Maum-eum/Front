...
export function RegionSelect2({ selectedLocations, setSelectedLocations, address, setAddress }: RegionSelectProps) {
			...
			<div className="text-sm flex px-1">
				...
				<div className="w-full px-3">
					...
					{location?.map((el) => (
						...
						<input
							type="checkbox"
							checked={selectedLocations === el.locationId}
							className="mr-2"
						/>
						{el.dongName}
						</li>
				...
				<div className="flex flex-col gap-2 px-1">
					<p className="text-sm bg-gray-200 font-gtr-R px-5 py-1 mt-1 rounded-xl">
						{sido?.find((el) => el.sidoId === selectedSido)?.sidoName + " "}
						{(sigungu?.find((el) => el.sigunguId === selectedSigungu)?.sigunguName || "")+ " "}
						{location?.find((el) => el.locationId === selectedLocations)?.dongName}
					</p>
					<p className="text-sm bg-gray-200 font-gtr-R px-5 py-1 mt-1 rounded-xl">
						<input
							type="text"
